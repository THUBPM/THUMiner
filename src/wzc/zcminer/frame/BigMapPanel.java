...
//map面板
public class BigMapPanel extends JPanel implements ComponentListener {
	//参数滑块
		...
		animationSlider.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				if(animationFlag == 1)
				{
					...
					currentFrame = MainFrame.bigAnimation.getAnimationFrame(animationSlider.getValue());
					if(pauseFlag == 0)
						startAnimation();
					else
						paintAnimation();
		...
		animationButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (animationFlag == 0){
					...
					currentFrame = MainFrame.bigAnimation.getAnimationFrame(0);
					if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
					{
		...
		timerTask = new TimerTask() {  
            public void run() {
            	...
            	if (currentTime >= MainFrame.graphNet.endTime){
            		...
            		Frame nextFrame = MainFrame.bigAnimation.getNext(currentFrame);
            		if(nextFrame.getFrame() < currentTime * (1000*60*60)){
