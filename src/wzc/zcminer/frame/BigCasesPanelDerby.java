...
//cases面板
public class BigCasesPanelDerby extends JPanel implements ComponentListener {
	...
	public BigCasesPanelDerby() {
		...
		{
		    variantsLabelPanel.add(new JLabel("Variants(" + MainFrame.bigVariantCollectionDerby.getSize() + ")"));
		}
		else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
		{
		    variantsLabelPanel.add(new JLabel("实例种类(" + MainFrame.bigVariantCollectionDerby.getSize() + ")"));
		}
		
		variantsListModel = new DefaultListModel<String>();
		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
		{
			variantsListModel.addElement("Complete Log(" + MainFrame.bigCaseCollectionDerby.getSize() + " cases)");
		}
		else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
		{
			variantsListModel.addElement("所有实例(" + MainFrame.bigCaseCollectionDerby.getSize() + "实例)");
		}
		for(int i = 0; i < MainFrame.bigVariantCollectionDerby.getSize(); i++)
		{
			...
			if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
			{
			    variantsListModel.addElement("Variant " + variant.getVariant() + " (" + variant.getSize() + "实例(" +
			            (new DecimalFormat("#0.00").format(1.0 * variant.getSize() / MainFrame.bigCaseCollectionDerby.getSize() * 100)) + "%))");
			}
			else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
			{
		...
		variantsList.addListSelectionListener(new ListSelectionListener(){
		    public void valueChanged(ListSelectionEvent e) {
		        if (e.getValueIsAdjusting() == false) {
    	            ...
    	            {
    	                casesListModel = new DefaultListModel<String>();
    	                BigVariantDerby variant = MainFrame.bigVariantCollectionDerby.getVariant(selectedValue - 1);
    	                caseComboBox.removeAllItems();
    	                ...
    	                for(int i = pageSelectedValue * 1000; i < (pageSelectedValue + 1) * 1000; i++)
    	                {
                            ...
                            BigCaseDerby mycase = variant.getCase(i);
    	        			if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
    	        			{
                                casesListModel.addElement(mycase.getCase() + " (" + mycase.getSize() + " events)");
    	        			}
		...
		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
		{
	        casesLabelPanel.add(new JLabel("Cases(" + MainFrame.bigCaseCollectionDerby.getSize() + ")"));
		}
		else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
		{
	        casesLabelPanel.add(new JLabel("实例(" + MainFrame.bigCaseCollectionDerby.getSize() + ")"));
		}
        ...
        creatChart(MainFrame.bigCaseCollectionDerby.getFirstID());
        refresh();
	}
    ...
    public void refresh()
    {
        ...
        {
            BigVariantDerby variant = MainFrame.bigVariantCollectionDerby.getVariant(variantSelectedValue - 1);
            BigCaseDerby mycase = variant.getCase(pageSelectedValue * 1000 + caseSelectedValue);
            updateTable(mycase);
            updateGraph(mycase);
            updateLabel(mycase);
            updateChart(mycase);
    ...
    public void creatChart(BigCaseDerby mycase)
    {
    ...
    public void updateChart(BigCaseDerby mycase)
    {
    ...
    public void updateLabel(BigCaseDerby mycase)
    {
		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
		{
    ...
    public void updateTable(BigCaseDerby mycase)
    {
        String[][] tableData = new String[mycase.getSize()][5];
        for(int i = 0; i < mycase.getSize(); i++)
        {
            BigEventDerby event = mycase.getEvent(i);
    ...
    public void updateGraph(BigCaseDerby mycase)
    {
