...
//statics面板
public class BigStaticsPanelDerby extends JPanel implements ComponentListener {
	...
	public BigStaticsPanelDerby() {
		...
		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
		{
			overviewTablePanel.addTab("Cases (" + MainFrame.bigCaseCollectionDerby.getSize() + ")", null, overviewCasePanel, "Cases");
			overviewTablePanel.addTab("Variants (" + MainFrame.bigVariantCollectionDerby.getSize() + ")", null, overviewVariantTablePanel, "Variants");
		}
		else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
		{
			overviewTablePanel.addTab("实例(" + MainFrame.bigCaseCollectionDerby.getSize() + ")", null, overviewCasePanel, "实例");
			overviewTablePanel.addTab("实例种类(" + MainFrame.bigVariantCollectionDerby.getSize() + ")", null, overviewVariantTablePanel, "实例种类");
		}
...
//	 	}
    ...
    {
        for(int i = 0; i <= (MainFrame.bigCaseCollectionDerby.getSize() - 1) / 1000; i++){
    ...
    {
        int size = MainFrame.bigVariantCollectionDerby.getSize();
        if(size > 1000)
            size = 1000;
        String[][] tableData = new String[size][5];
        for(int i = 0; i < size; i++)
        {
            BigVariantDerby variant = MainFrame.bigVariantCollectionDerby.getVariant(i);
    		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
    		{
    ...
    public void updateOverviewLabel()
    {
		if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
		{
	        overviewEventsLabel.setText("Events : " + MainFrame.bigEventCollectionDerby.getSize());
	        overviewCasesLabel.setText("Cases : " + MainFrame.bigCaseCollectionDerby.getSize());
	        overviewActivitiesLabel.setText("Activities : " + (MainFrame.graphNet.activityCount - 2));
	        long median = MainFrame.bigCaseCollectionDerby.getMedianDuration();
	        long medianMinutes = median / (60 * 1000) % 60;
	        long medianHours = median / (60 * 60 * 1000) % 24;
	        long medianDays = median / (24 * 60 * 60 * 1000);
	        overviewMedianCaseDurationLabel.setText("Median case duration : " + medianDays + " days " + medianHours + " hours " + medianMinutes + " mins");
	        long mean = MainFrame.bigCaseCollectionDerby.getMeanDuration();
	        long meanMinutes = mean / (60 * 1000) % 60;
	        long meanHours = mean / (60 * 60 * 1000) % 24;
	        long meanDays = mean / (24 * 60 * 60 * 1000);
	        overviewMeanCaseDurationLabel.setText("Mean case duration : " + meanDays + " days " + meanHours + " hours " + meanMinutes + " mins");
	        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	        overviewStartLabel.setText("Start : " + formatter.format(MainFrame.bigCaseCollectionDerby.getStart()));
	        overviewEndLabel.setText("End : " + formatter.format(MainFrame.bigCaseCollectionDerby.getEnd()));
        }
		else if(MainFrame.properties.getProperty("language", "zhCN").equals("zhCN"))
		{
	        overviewEventsLabel.setText("事件数：" + MainFrame.bigEventCollectionDerby.getSize());
	        overviewCasesLabel.setText("实例数：" + MainFrame.bigCaseCollectionDerby.getSize());
	        ...
	        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	        overviewStartLabel.setText("开始时间：" + formatter.format(MainFrame.bigCaseCollectionDerby.getStart()));
	        overviewEndLabel.setText("结束时间：" + formatter.format(MainFrame.bigCaseCollectionDerby.getEnd()));
	 	}
    }
    ...
    public void updateOverviewCaseVariantsChart()
    {
        ...
        for(int i = 0; i < MainFrame.bigVariantCollectionDerby.getSize(); i++)
        {
            BigVariantDerby variant = MainFrame.bigVariantCollectionDerby.getVariant(i);
    ...
    public void updateActivityAllActivitiesTable()
    {
        ...
        for(int i = 0; i < size; i++)
        {
            ...
            tableData[i][2] = new DecimalFormat("#0.00").format(activity.getFrequency() * 1.0 / MainFrame.bigEventCollectionDerby.getSize() * 100) + "%";
            if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
    		{
    ...
    public void updateActivityFirstInCaseTable()
    {
        ...
        for(int i = 0; j < size; i++)
        {
            ...
            if(activity.getFirst())
            {
                ...
                tableData[j][2] = new DecimalFormat("#0.00").format(activity.getFrequency() * 1.0 / MainFrame.bigEventCollectionDerby.getSize() * 100) + "%";
                if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
        		{
                    long median = activity.getMedianDuration();
                    long medianMinutes = median / (60 * 1000) % 60;
    ...
    public void updateResourceAllResourcesTable()
    {
        ...
        for(int i = 0; i < size; i++)
        {
            ...
            tableData[i][2] = new DecimalFormat("#0.00").format(resource.getFrequency() * 1.0 / MainFrame.bigEventCollectionDerby.getSize() * 100) + "%";
            if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
    		{
    ...
    public void updateResourceFirstInCaseTable()
    {
        ...
        for(int i = 0; j < size; i++)
        {
            ...
            if(resource.getFirst())
            {
                ...
                tableData[j][2] = new DecimalFormat("#0.00").format(resource.getFrequency() * 1.0 / MainFrame.bigEventCollectionDerby.getSize() * 100) + "%";
                if(MainFrame.properties.getProperty("language", "zhCN").equals("enUS"))
        		{
                    long median = resource.getMedianDuration();
                    long medianMinutes = median / (60 * 1000) % 60;
